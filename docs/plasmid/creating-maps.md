...
<!-- prettier-ignore -->
!!! note
  The map tools have been moved to a separate mod called "Nucleoid Creator Tools", you can install those tools [here](https://modrinth.com/mod/nucleoid-creator-tools).
...
An empty void map workspace can be opened by running: `/map open <id>` (e.g. `/map open examplegame:main`)  
...
You are also able to create workspaces with custom chunk generators: `/map open <id> like <dimension>` (e.g. `/map open examplegame:nether like minecraft:nether`)
...
For example, this is what it would look like if you created a 1x1 region named spawn:
![A screenshot of a stone box with one spawn region inside it](../assets/images/map-creation/created_region.png)
